...
// Cek agar tidak double initialize jika reload
...
const app = {
    ...
    // --- FUNGSI SINKRONISASI CLOUD (DIPERCEPAT) ---
    async saveDB() {
        // 1. Simpan ke Local DULU (Instan)
        localStorage.setItem('MMRC_DATABASE', JSON.stringify(this.data));
        
        // 2. Simpan ke Firebase (Background Process)
        // Kita tidak memakai 'await' yang memblokir UI, biarkan dia jalan di background
        db.ref('mmrc_data').set(this.data)
            .then(() => console.log("Data Tersinkron ke Cloud"))
            .catch((e) => console.error("Gagal Sinkron Cloud (Tapi Local Aman):", e));
    },

    // --- LOGIKA LOAD DB BARU (ANTI LOADING) ---
    async loadDB() {
        // LANGKAH 1: Langsung ambil dari memori HP (Local Storage)
        // Ini membuat aplikasi terbuka INSTAN tanpa loading screen
        const local = localStorage.getItem('MMRC_DATABASE');
        if (local) {
            try {
                this.data = JSON.parse(local);
                this.render(); // Langsung render tampilan!
            } catch (e) {
                console.error("Data local corrupt", e);
            }
        }

        // LANGKAH 2: Ambil data terbaru dari Cloud di Background (Tanpa Loading Screen Mengganggu)
        // User sudah bisa pakai aplikasi sambil menunggu data terbaru turun
        try {
            const snapshot = await db.ref('mmrc_data').once('value');
            const cloudData = snapshot.val();
            
            if (cloudData) {
                // Update data dengan yang terbaru dari cloud
                this.data = cloudData;
                // Simpan lagi ke local untuk pembukaan berikutnya
                localStorage.setItem('MMRC_DATABASE', JSON.stringify(cloudData));
                
                // Render ulang diam-diam untuk update data terbaru
                this.render(); 
                console.log("Data Cloud Berhasil Di-update di Background");
            }
        } catch (e) {
            console.error("Koneksi Offline/Lambat: Menggunakan data terakhir di HP");
            // Tidak perlu error alert, karena user sudah bisa pakai data local
        }
    }, 
    ...
    render() {
        ...
        if (!container) return;
        // Kita tidak mengosongkan innerHTML di awal jika tidak perlu untuk mencegah kedip
    ...
    // INI LOGIKA YANG ANDA MINTA: Hapus Log -> Stok Balik
        ...
        document.getElementById('modal-body').innerHTML = `
            <div class="space-y-4">
                <select id="pr_type" class="input-field">
                    <option value="Reguler">Reguler</option><option value="Eksklusif">Eksklusif</option>
                </select>
                <select id="pr_dur" class="input-field">
                    <option value="7 Hari">7 Hari</option><option value="14 Hari">14 Hari</option><option value="30 Hari">30 Hari</option>
                </select>
    ...
    // Fungsi export Word sederhana
    exportToWord() {
       // Logic sederhana atau placeholder jika library docx belum diset full
       alert("Fitur Export Word Aktif");
    }
};

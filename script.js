...
const app = {
    ...
    login() {
        ...
        if (u === 'OPERASIONAL.MMRC' && p === 'MADANI1999') {
            ...
            Swal.fire({
                title: 'System Access',
                html: 'Authenticating credentials...',
                timer: 800,
            ...
            Swal.fire({ icon: 'error', title: 'Access Denied', text: 'Invalid credentials provided.', showConfirmButton: false, timer: 1500 });
        }
    },
    ...
    renderDashboard(c) {
        ...
        c.innerHTML = `
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                ...
                ${this.card('DRUG ITEMS', stats.drugs, 'text-orange-600')}
            </div>
    ...
    async pushData() {
        ...
        if (!n || !r) return Swal.fire('Validation Error', 'Name and RM are mandatory fields.', 'warning');
        ...
        c.innerHTML = `
            <div class="bg-white rounded-[2rem] shadow-sm border border-slate-100 overflow-hidden flex flex-col h-full">
                <div class="p-6 border-b border-slate-100 flex gap-4 items-center bg-slate-50/30">
                    ...
                    <input type="text" id="g_search" onkeyup="app.filter()" placeholder="Quick Search..." class="bg-transparent w-full outline-none font-medium text-slate-600">
                </div>
    ...
    async drop(i) {
        const r = await Swal.fire({ title: 'Delete Record?', text: "This action is irreversible.", icon: 'warning', showCancelButton: true, confirmButtonColor: '#d33', confirmButtonText: 'Yes, Delete' });
        if (r.isConfirmed) {
    ...
    detail(i) {
        ...
        document.getElementById('modal-title').innerText = p.reg.name.toUpperCase();
